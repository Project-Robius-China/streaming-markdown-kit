...
- Language-aware bypass: inline rules (emphasis, inline code, math, strike) are bypassed inside fenced blocks whose info-string (lowercased, trimmed) is in the compile-time set `BYPASS_LANGUAGES = {"mermaid", "math", "tex", "latex", "typst", "asciidoc"}`. Only the fenced-code closer rule applies inside these blocks. The set is a `const` in `src/remend.rs`, not a runtime knob in M1, to keep the M1 API surface minimal.
